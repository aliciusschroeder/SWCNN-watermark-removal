...
def prepare_data(
    ...
    print(f"Creating patches of size {patch_size} with stride" +
          f"{stride} and {aug_times - 1} additional augmentation times")
    print(f"Will repeat that process for the following scales: {scales}")
    train_files_path, train_files, train_h5f_path = input_files(
        data_path, 'train', mode)
    val_files_path, val_files, val_h5f_path = input_files(
    ...
    if min(len(train_files), len(val_files)) < 0:
        print(f"Error: Found only {len(train_files)} training files and" +
