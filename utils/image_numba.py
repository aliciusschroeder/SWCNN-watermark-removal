...
COLOR_SPACE_PARAMS = {
    "srgb_to_linear": {
        # Transition point between linear and non-linear scaling in sRGB.
        "threshold": 0.04045,

        # Scaling factor for linear colors below the threshold.
        "scale_factor": 12.92,

        # Gamma correction exponent for non-linear colors.
        "gamma": 2.4,

        # Offset added to normalize non-linear scaling.
        "offset": 0.055,

        # Multiplier for non-linear scaling adjustment.
        "multiplier": 1.055,
    },
    "linear_to_srgb": {
        # Transition point between linear and non-linear scaling in linear space.
        "threshold": 0.0031308,

        # Scaling factor for linear colors below the threshold.
        "scale_factor": 12.92,

        # Inverse gamma correction for non-linear colors.
        "gamma": 2.4,

        # Offset to normalize sRGB scaling.
        "offset": 0.055,

        # Multiplier to match the sRGB curve.
        "multiplier": 1.055,
    },
}
...
def srgb_to_linear(color: np.ndarray, randomize: bool = False) -> np.ndarray:
    if color.ndim != 3:
        raise ValueError(
            f"Input color array must be 3D (H, W, C), but got shape {color.shape}")
    ...
    # Launch kernel
    srgb_to_linear_cuda[blocks_per_grid, threads_per_block](  # type: ignore
        d_color,
        d_result,
        params["threshold"],
        params["scale_factor"],
...
def linear_to_srgb(color: np.ndarray, randomize: bool = False) -> np.ndarray:
    ...
    # Launch kernel
    linear_to_srgb_cuda[blocks_per_grid, threads_per_block](  # type: ignore
        d_color, d_result,
        params["threshold"],
        params["scale_factor"],
        params["gamma"],
...
if __name__ == "__main__":
    ...
    # Example dimensions (e.g., 1080p image with 3 channels)
    H, W, C = 1080, 1920, 3
    ...
    for _ in range(iterations):
        ...
        restored_colors_cuda = linear_to_srgb(
            linear_colors_cuda, randomize=False)
        current_max_diff_cuda = np.max(
            np.abs(test_colors - restored_colors_cuda))
        ...
        current_max_diff_numpy = np.max(
            np.abs(test_colors - restored_colors_numpy))
        max_difference_numpy = max(
            max_difference_numpy, current_max_diff_numpy)

    # Benchmarking CUDA functions
    benchmark_runs = 100  # Reduced to a reasonable number for demonstration
    mean_time_stl_cuda, std_dev_stl = benchmark_function(
        srgb_to_linear, test_colors, test_runs=benchmark_runs)
    mean_time_lts_cuda, std_dev_lts = benchmark_function(
        linear_to_srgb, test_colors, test_runs=benchmark_runs)

    print("== CUDA-Version ==")
    print("Original shape:", test_colors.shape)
    print("Max difference:", max_difference_cuda)
    print(f"Mean time srgb_to_linear: {mean_time_stl_cuda:.6f}" +
          f" ± {std_dev_stl:.6f} seconds")
    print(f"Mean time linear_to_srgb: {mean_time_lts_cuda:.6f}" +
          f" ± {std_dev_lts:.6f} seconds")
    print("\n")

    # Benchmarking Numpy functions
    mean_time_stl_numpy, std_dev_stl_numpy = benchmark_function(
        stl, test_colors, warmup_runs=3, test_runs=10)
    mean_time_lts_numpy, std_dev_lts_numpy = benchmark_function(
        lts, test_colors, warmup_runs=3, test_runs=10)

    print("== Numpy-Version ==")
    print("Original shape:", test_colors.shape)
    print("Max difference:", max_difference_numpy)
    print(f"Mean time srgb_to_linear: {mean_time_stl_numpy:.6f} " +
          f"± {std_dev_stl_numpy:.6f} seconds")
    print(f"Mean time linear_to_srgb: {mean_time_lts_numpy:.6f} " +
          f"± {std_dev_lts_numpy:.6f} seconds")
    print("\n")
